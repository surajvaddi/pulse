...
const swappable = await service.listSwappableShifts(session("user_priya"));
...
const { originalShift, decision } = await service.evaluateOriginalShift(session("user_priya"), "slot_shift_priya_future_swap_assertion");
assert.equal(decision.allowed, true);
assert.equal(originalShift.employeeId, "emp_priya");

const candidates = await service.listCandidates(session("user_priya"), "slot_shift_priya_future_swap_assertion");
